...
export default function PostViewItem({ post }) {
    ...
    return (
        <IonCard className="post-card-full-width" color="dark">
            ...
            <IonCardContent>
                <h1>{post?.title ? post.title : "No title specified"}</h1>
                <p>{post?.body ? post.body : "No description specified"}</p>
                <p>{post?.keywords ? (<i>Keywords: {post.keywords}</i>) : "No keywords specified"}</p>
            </IonCardContent>
        </IonCard>
    );
};